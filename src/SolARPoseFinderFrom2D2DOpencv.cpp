                                                            ...
                                                            std::vector<DescriptorMatch> & inlierMatches)
{
    ...
    LOG_DEBUG("Estimated pose of poseView1: \n {}", poseView1.matrix());
    LOG_DEBUG("Estimated pose of poseView1Inverse: \n {}", poseView1Inverse.matrix());
    ...
    cv::Point2f pp; 
	pp.x = camParams.intrinsic(0,2); 
	pp.y = camParams.intrinsic(1,2);
    cv::Mat Ecv = cv::findEssentialMat(points_view1, points_view2, camParams.intrinsic(0,0), pp, cv::RANSAC, m_confidence, m_outlierDistanceRatio * maxVal, inliers);
    cv::Mat cvRot;
    cv::Mat cvPos;

    cv::recoverPose(Ecv, points_view1, points_view2, cvRot, cvPos, camParams.intrinsic(0,0), pp, inliers);
    ...
    LOG_DEBUG("Estimated pose of poseView2: \n {}", poseView2.matrix());
...
FrameworkReturnCode SolARPoseFinderFrom2D2DOpencv::estimate(const std::vector<Keypoint> & matchedPointsView1,
                                                            ...
                                                            std::vector<DescriptorMatch> & inlierMatches)
{
	std::vector<Point2Df> pts1, pts2;
	for (int i = 0; i < matchedPointsView1.size(); ++i) {
		pts1.push_back(matchedPointsView1[i]);
		pts2.push_back(matchedPointsView2[i]);
	}
    return estimate(pts1, pts2, camParams, poseView1, poseView2, inlierMatches);
}
