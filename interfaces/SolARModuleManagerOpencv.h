...
const string POSE_ESTIMATION_PNP="0753ade1-7932-4e29-a71c-66155e309a53";
const string POSE_ESTIMATION_PNP_EPFL="a38edf79-f0dc-45ca-92fc-2b336fceedf9";
...
int SolARModuleManagerOpencv::createComponent(string uuid, SRef<T> &compRef)
{
    ...
    else if (uuid == UUID::POSE_ESTIMATION_PNP) // Pose estimation component based on simple Opencv implementation
    {
        res=m_xpcfComponentManager->createComponent(gen(uuid), gen(api::solver::pose::I3DTransformFinder::UUID), compRef);
        if (res == -1)
             LOG_ERROR("PnP Pose estimation component creation has failed");
        return res;
    }

    else if (uuid == UUID::POSE_ESTIMATION_PNP_EPFL) // Pose estimation component based on EPFL implementation
    {
        res=m_xpcfComponentManager->createComponent(gen(uuid), gen(api::solver::pose::I3DTransformFinder::UUID), compRef);
        if (res == -1)
             LOG_ERROR("EPFL PnP Pose estimation component creation has failed");
        return res;
    }
