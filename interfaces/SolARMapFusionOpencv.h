...
public:
	...
	FrameworkReturnCode merge(SRef<datastructure::Map> map,
...
private:
	...
	/// @param[in,out] map local map or floating map to merge
	/// @param[in,out] globalMap the global map		
    void fuseMap(const std::vector<std::pair<uint32_t, uint32_t>>&cpOverlapIndices, SRef<datastructure::Map> map, SRef<datastructure::Map> globalMap);
